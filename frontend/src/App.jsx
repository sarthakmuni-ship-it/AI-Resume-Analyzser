...
export default function App() {
  ...
  const handleDownloadReport = () => {
    ...
    const addText = (text, isHeading = false) => {
      ...
      if (isHeading) {
        doc.setTextColor(37, 99, 235); // Blue heading
      } else {
    ...
    addText(`RÉSUMÉ ATS AUDIT REPORT`, true);
    addText(`Date Generated: ${new Date().toLocaleString()}`);
    addText(`Match Score: ${analysisData.match_score}% / 100\n`);

    addText(`MATCHED KEYWORDS`, true);
    addText(analysisData.matched_skills?.length 
      ? analysisData.matched_skills.join(', ') 
      : 'None found');

    addText(`\nMISSING KEYWORDS`, true);
    const missingText = analysisData.missing_skills?.length 
    ...
    const feedbackText = analysisData.ats_feedback?.length 
      ? analysisData.ats_feedback.map(f => `• ${f}`).join('\n') 
      : 'No feedback provided.';
    addText(feedbackText);

    addText(`\nAI SUGGESTED REWRITES`, true);
    if (analysisData.rewritten_bullets?.length) {
      analysisData.rewritten_bullets.forEach((b, i) => {
        addText(`[Suggestion ${i + 1}]`, true);
        addText(`Before: ${b.original}`);
        addText(`After:  ${b.rewritten}\n`);
      });
    } else {
      addText('No rewrites suggested.');
    }
